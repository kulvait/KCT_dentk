...
int main(int argc, char* argv[])
{
    ...
    if (a.grayscaleSpecified) {
        std::shared_ptr<io::Chunk2DReaderItkI<float>> sliceReader = std::make_shared<io::DenChunk2DReaderItk<float>>(a.input_file);
        framesToOutput = processResultingFrames(a.frames, sliceReader->count());
        for (int i = 0; i != framesToOutput.size(); i++) {
            std::string tmpImg = io::xprintf("%s/%s%03d.bmp", a.output_directory.c_str(), a.file_prefix.c_str(), framesToOutput[i]);
            if (a.intervalSpecified) {
                io::writeCastedImage<float>(sliceReader->readChunk2DAsItkImage(framesToOutput[i]), tmpImg, a.min, a.max);
            } else {
                io::writeCastedImage<float>(sliceReader->readChunk2DAsItkImage(framesToOutput[i]), tmpImg);
            }
        }
        ...
        std::shared_ptr<io::Chunk2DReaderI<float>> redSliceReader, greenSliceReader, blueSliceReader;
        redSliceReader = NULL;
...
int Args::parseArguments(int argc, char* argv[])
{
    ...
    app.add_option("-R,--red-component", input_file_red, "File in a DEN format to be interpretted as a red component.");
    app.add_option("-B,--blue-component", input_file_blue, "File in a DEN format to be interpretted as a blue component.");
    app.add_option("-G,--green-component", input_file_green, "File in a DEN format to be interpretted as a green component.");
    app.add_option("-f,--frames", frames, "Specify only particular frames to process. You can input range i.e. 0-20 or also individual coma separated frames i.e. 1,8,9. Order does matter. Accepts end literal that means total number of slices of the input.");
    app.add_option("-d,--output-dir-name", output_directory, "Output directory. Defaults to '/tmp'.")->check(CLI::ExistingDirectory);
    app.add_option("-p,--file-prefix", file_prefix, "Prefix of files to write, defaults to slice.");
    CLI::Option* minopt = app.add_option("--min", min, "Float value that will be interpreted as a minimum for interpolation [min,max] -> [0,255].")->check(CLI::Range(0.0, 1000.0));
    CLI::Option* maxopt = app.add_option("--max", max, "Float value that will be interpreted as a maximum for interpolation [min,max] -> [0,255].")->needs(minopt)->check(CLI::Range(0.0, 1000.0));
    minopt->needs(maxopt);
    app.add_option("den_file", input_file, "File in a DEN format to produce a grayscale image. If specified, no other (R, G, B) components are processed.")->check(CLI::ExistingFile);
    try {
        app.parse(argc, argv);
    ...
    if (app.count("--red-component") > 0 || app.count("--blue-component") > 0 || app.count("--green-component") > 0)
