...
int main(int argc, char* argv[])
{
    ...
    std::shared_ptr<io::Chunk2DReaderI<float>> denSliceReader = std::make_shared<io::DenChunk2DReader<float>>(a.input_file);
    ...
    std::shared_ptr<io::AsyncImageWritterI<float>> imagesWritter = std::make_shared<io::DenAsyncWritter<float>>(a.output_file, dimx, dimy, framesToOutput.size());
...
int Args::parseArguments(int argc, char* argv[])
{
    ...
    app.add_flag("-r,--reverse_order", reverse_order, "Output in the reverse order of input or reverse specified frames.");
    ...
    app.add_option("-k,--each-kth", k, "Process only each k-th frame specified by k to output. The frames to output are then 1st specified, 1+kN, N=1...\\infty if such frame exists. Parametter k must be positive integer.")->check(CLI::Range(1, 65535));
    app.add_option("-j,--threads", threads, "Number of extra threads that application can use.")->check(CLI::Range(1, 65535));
    ...
    LOGD << io::xprintf("Input file is %s and output file is %s.", input_file.c_str(), output_file.c_str());
    try {
        app.parse(argc, argv);
